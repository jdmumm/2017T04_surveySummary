...
awl <- read.csv ("./data/931_CHCW_T04T05T06_usedInEst_90to17.csv")
str(awl)
  
awl %>% filter (PROJECT_CODE == "T04", YEAR == "2017", SEX_CODE == '1') %>%  
...
plot (ch_ln ~ cw_ln, data =len, 
  ...
  abline(lm_s, col = 'red', lty = 'dashed' )
  abline(lm_l, col = 'blue', lty = 'dashed')
...
len %>%  ggplot(aes(rat))+geom_density(alpha=.2)
