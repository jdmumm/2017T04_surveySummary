...
Legal Male abundance using the new size classes. 
...
Legal Male abundance using the old size classes. 
```{r, results='asis', echo=F}
...
select(Year = YEAR, Tows = n, 'Pre-4 (<70mm)' = P4, 'Pre-3 (70-91mm)' = P3, 'Pre-2 (92-114mm)' = P2, 'Pre-1 (115-139mm)' = P1,
                  'Legal (>404mm)'= LM_P_, 'Legal CI'= LM_P_CI_, 'Total' = TM_P_, 'Total CI'= TM_P_CI_ ) %>%
mutate (Year = as.factor(Year)) %>%
mutate_if(is.numeric,funs(prettyNum(., big.mark=","))) -> LM17
print(xtable(LM17,
